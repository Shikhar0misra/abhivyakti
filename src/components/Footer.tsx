...
const Footer = () => {
  return (
    <footer id="footer" className="py-16 border-t border-border grain-overlay">
      <div className="container mx-auto px-6 relative z-10">
        <div className="grid md:grid-cols-3 gap-12 items-start">
          <div>
            <div className="flex items-center gap-3 mb-4">
              <img src={theatreMasks} alt="Logo" className="w-8 h-8" />
          ...
          <div>
            ...
            <div className="space-y-2">
              {["Home", "Events", "About Us"].map((link) => (
                <button
                  key={link}
                  onClick={() => document.getElementById(link === "Events" ? "ilhaam" : link === "About Us" ? "about" : "home")?.scrollIntoView({ behavior: "smooth" })}
                  className="block text-muted-foreground font-body text-sm hover:text-foreground transition-colors"
                >
                  {link}
                </button>
              ))}
          ...
          <div>
            <h4 className="font-heading text-lg font-semibold text-foreground mb-4">Connect</h4>
